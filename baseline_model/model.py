...
from baseline_model.data_handler import DataHandler
...
class CNNBaselineModel:
    ...
    def _determine_fc_input_features(self):
        ...
        conv_part = nn.Sequential(*list(self.__model.children())[:-2])

        with torch.no_grad():
            dummy_output = conv_part(dummy_input)

        flattened_features = dummy_output.numel()
        self.__model[-2] = nn.Linear(in_features=flattened_features, out_features=self.__num_classes).to(self.__device)
    ...
    def evaluate(self):
        ...
        with torch.inference_mode():
            for X, y in self.__test_dataloader:
                ...
                loss = self.__loss_fn(outputs, y)
                test_loss += loss.item()

                pred_labels = torch.argmax(outputs, dim=1)
                test_acc += (pred_labels == y).sum().item() / len(outputs)
    ...
    def predict(self, image_tensor: torch.Tensor):
        ...
        with torch.inference_mode():
            ...
            output_probs = self.__model(image_tensor)
